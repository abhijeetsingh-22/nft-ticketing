...
export default function AllEvents({ initialEvents }: { initialEvents: Event[] }) {
  ...
  const handleBuyEventTicket = async (eventId: string) => {
    ...
    toast.promise(
      buyEventTicket(selectedEvent, publicKey, connection, balance, signTransaction),
      {
        loading: 'Processing payment...',
        success: 'Ticket purchased successfully! Please check your wallet for your NFT',
      ...
      <section className="space-y-6 w-full lg:w-3/4">
        ...
        <div className="space-y-6">
          {filteredEvents.map((event) => (
            <EventCard key={event.id} {...event} handleBuyEventTicket={handleBuyEventTicket} />
          ))}
